...
- E2e: the family's pages mount real React trees in the switch's React view
  and pass axe.
- **Every data-driven `href`/`src` goes through `sanitizeUrl` /
  `sanitizeResourceUrl`.** Angular gets URL sanitizing from `DomSanitizer`;
  React does not, so a `url`-shaped prop written plain is an XSS sink in the
  React layer only — a parity defect that looks like working code. Check this
  on any component with a `url`, `imageOf` or thumbnail prop.
- **Anything Angular does on the _initial_ value write, React must do at
  mount.** An effect keyed on the value only fires on a later change, so a
  component seeded from `useState(() => …today…)` silently disagrees with the
  Angular one — how `<OgeCalendar>` came to open on the wrong month for eleven
  months of the year.
...
| Data Grid  | 🟡 slices A+B+C+D: everything in A/B plus **editing** (cell, row, batch, popup, form; command column, `addRow`/`editRow`/`deleteRow`/`saveChanges`/`discardChanges`/`hasChanges`, the ten editing events, per-column `editable`/`required`/`validators`/`renderEditor`), header filter, filter panel + visual filter builder, `filterValue`, column chooser, column bands, row/header context menus, `highlightChanges`, deferred selection | 🟡 overview, api, grouping, master-detail, rows branch; columns, filtering, selection, editing, persistence, context-menu pages show the shell notice | ✅ / ✅           | ⏳ not in `FAMILIES` until the docs pages branch (see the R6 phase table below) |
...
- **The data grid, sliced (2026-08-23).** `@oge-ui/react-grid` ships the
  ...
  `addRow`/`editRow`/…) and **D** (header filter, filter builder/panel, column
  chooser, column bands, context menus, `highlightChanges`, deferred
  selection), plus the `export-excel` / `export-pdf` entries, have landed —
  every one of them on the same behavior cores the Angular grid runs,
  `OgeGridEditingCore` included, whose editor bridge was built for exactly
  this split: Angular fills it with `FormControl`s, React with draft state.
  The exporters followed the sharing rule rather than the shortcut: the pure
  `buildExcelWorkbook` / `buildPdfDocument` moved into
  `@oge-ui/behavior/export-excel` and `/export-pdf` (optional `exceljs` /
  `jspdf` peers), and both grid packages now keep only the three lines that
  turn a grid into a download and re-export the builder, so neither public API
  changed. What is left before the family joins `FAMILIES` is the **docs
  pages**: columns, filtering, selection, editing, persistence and
  context-menu still show the React shell notice instead of branching. Until
  then the React API table documents exactly what ships.
