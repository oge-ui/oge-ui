...
| `grid`        | `packages/grid`      | `@oge-ui/grid`      | Data grid. Secondary entries: `/foundation`, `/export-excel`, `/export-pdf`. Owns the design tokens and theme CSS.                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `inputs`      | `packages/inputs`    | `@oge-ui/inputs`    | Form editors (OgeTextBox/OgeTextArea/OgeNumberBox), dropdown editors (OgeSelectBox/OgeTagBox/OgeAutocomplete) and bare toggle controls (OgeCheckBox/OgeSwitch/OgeRadioGroup). Base split: `OgeControlBase` (chrome-free — commit pipeline, CVA constructor-assignment, Signal Forms `FormValueControl`) → `OgeInputBase` (adds the field chrome). Dropdown editors share `lib/select-list/` (`SelectListEngine`, `SelectPanelController`, `ListVirtualizerModel` on core's `OffsetTree`, `expr.ts` resolvers). |
