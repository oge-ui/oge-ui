...
| `navigation`  | `packages/navigation` | `@oge-ui/navigation` | Navigation controls. Today `OgeTreeView` (WAI-ARIA APG treeview: roving tabindex, Right/Left open-child / close-parent semantics, type-ahead, `*`; flat **or** nested data; tri-state cascade checkboxes; search; lazy `loadChildren`; virtual scrolling; drag & drop reparenting). The tree needed no new data code — core's `lib/tree/` engine was already there from tree-list. Also `OgeDrawer` (overlay/push/side, with modality derived from the mode) and `OgeStepper` + `OgeStep` (no APG stepper pattern exists, so an ordered list of `<button>` headers carrying `aria-current="step"` with `role="region"` panels — one semantic in both orientations, unlike Material). Depends on `core` and, since the drawer, `overlay`. Future home for Menubar / Breadcrumb.                                                                                                                                                                                                                                                                                                                                                                                                                            |
