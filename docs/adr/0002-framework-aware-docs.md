...
| 8   | Component families without a React package | Nothing per page — the shell notice covers them. Revisited family by family as each React package lands: buttons, inputs, tabs, layout, navigation, forms, overlay and upload are whole; the data grid branches page by page as its slices ship (`docs/REACT-PARITY.md` is the status table). | ongoing |
...
- The `llms.txt` generator already branches on `platform`, so converting a page
  ...
  the pages.
- Item 8 was the honest admission that most of the suite was Angular-only
  when this was written. The shell notice makes the remaining gaps visible
  without a per-page apology, and it has been disappearing family by family
  as ADR 0001's Faz 5 proceeds (status: `docs/REACT-PARITY.md`).
