...
| 3   | ✅ **Done** (0.12–0.13: panel + menu, then tooltip / context menu / modal / toast). `overlay` split across both frameworks; positioning, focus trap, Escape stack, menu keyboard machine move into `behavior`                                                                                                  | **The real test.** Whether a shared layer is actually shared is only knowable with two consumers, and this fazda the Angular side gets refactored too. If the architecture is wrong, it is wrong here — with three packages ported, not fifteen.                                                                                                                |
| 4   | ✅ **Done** (`docs-tools:parity`, in CI). Cross-framework parity gate                                                                                                                                                                                                                                          | B's known failure mode is parity drift. The gate must land after the _second_ package, not the tenth.                                                                                                                                                                                                                                                           |
| 5   | 🟡 **In progress** — inputs, layout, tabs, navigation, forms, upload whole; grid in slices (A+B shipped), tree-list next. Scale out: `inputs` → `layout`/`tabs`/`navigation` → `forms` → `grid` → `tree-list`. In parallel: `bpmn` (no dependencies, 50% already engine) and `pivot` (depends on `core` alone) | Dependency order, with the two low-coupling packages free to run off the main chain.                                                                                                                                                                                                                                                                            |
