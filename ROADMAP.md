...
| Resources & grouping (`resources`, `groups`)                               | dx/K/FC   | covered | editor selects, color-by-resource, timeline rows, day/week column grouping (create prefill, drag reassign); `groupByDate`/orientation → backlog                      |
| Timeline (`timelineDay`/`timelineWeek`) + Agenda views                     | dx/K/FC   | covered | timeline bars via the transposed overlap kernel, drag + keyboard move/resize + row (resource) reassign; Year overview view with busy dots; `intervalCount` → backlog |
...
| Templates (task bar content)                                             | dx/K      | covered | `*ogeGanttTaskTemplate` + `*ogeGanttTooltipTemplate` (hover tooltip with rich default content)                                      |
...
| Work-time calendar (`workCalendar`: working weekdays + holidays)         | K         | covered | shades off days; auto-scheduling rolls starts onto working days, durations kept in working days; per-resource `calendar` overrides  |
| Export (Excel + drawn PDF + PNG)                                         | dx/K      | covered | lazy `export-excel` / `export-pdf` entries (optional exceljs/jspdf peers) + dependency-free `export-image` PNG                      |
...
## Upload (`@oge-ui/upload`) — Feature Parity

References swept: **dxFileUploader**, **Kendo Upload** (plus FileSelect and the
external drop-zone component), **Syncfusion Uploader**, **PrimeNG FileUpload**,
**Ant Design Upload**.

**Two absences recorded.** Angular Material and the CDK ship **no** file
uploader and no drop-zone primitive of any kind — there is nothing to map (the
dxCardView rule). And the **WAI-ARIA APG has no file-upload or dropzone
pattern**: the accessibility contract below is assembled from the button, list,
progressbar and live-region patterns, and that assembly is itself a decision,
recorded here rather than cited.

| Feature                                                                                      | Reference    | Status  | Notes                                                                                                                                                                                                                              |
| -------------------------------------------------------------------------------------------- | ------------ | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `accept`, `multiple`, `disabled`, `readonly`, `tabIndex`                                     | all          | Done    | `multiple` defaults to `true` — the majority of the references; dx's `false` is the outlier                                                                                                                                        |
| Directory upload                                                                             | Ant, SF      | Done    | `directory`; dropped folders walk `webkitGetAsEntry` with a files-only fallback                                                                                                                                                    |
| Paste to upload                                                                              | Ant          | Done    | `pastable`; reads `files` and falls back to `items`, which is where Safari puts a pasted screenshot                                                                                                                                |
| Drop zone on the component                                                                   | all          | Done    | The zone **is** a `<button>`, so one element is both the keyboard affordance and the drop target — no nested interactive, no `div` with a click handler                                                                            |
| External drop zone                                                                           | dx/K/SF      | Done    | `[ogeUploadDropZone]` plus a `dropZone` name (dx `dropZone`, Kendo `zoneId`, SF `dropArea`); a different element cannot be expressed as a mode value                                                                               |
| External trigger button                                                                      | dx           | Done    | `[ogeUploadTrigger]` — dx's `dialogTrigger`                                                                                                                                                                                        |
| `dropEffect`                                                                                 | SF           | Done    | Lowercased string union per the house rule                                                                                                                                                                                         |
| Extension / min / max size restrictions                                                      | all          | Done    | Flattened, not Kendo's grouped `FileRestrictions` object — flat inputs bind without an object literal that re-creates on every change                                                                                              |
| File-count limit                                                                             | PrimeNG, Ant | Done    | `maxFileCount`; only the files past the limit are rejected, and a rejected file never spends a count slot                                                                                                                          |
| Total-size limit                                                                             | —            | Done    | **OGE extra**                                                                                                                                                                                                                      |
| Custom per-file validation                                                                   | Ant          | Done    | `validateFile` — the validation half of `beforeUpload`; named around the `Validator.validate` method                                                                                                                               |
| Rejected files stay visible with their reason                                                | —            | Done    | **OGE extra as an event** (`fileRejected`); the references only render a message                                                                                                                                                   |
| Upload URL, method, headers, custom data, credentials, response type, timeout                | all          | Done    | `uploadUrl` also takes a function — Ant's `action` in its function form                                                                                                                                                            |
| `uploadMode`                                                                                 | dx           | Done    | `instantly` / `useButtons` / `useForm` / **`select`**. `useForm` uploads nothing itself and mirrors the rows back into the native input's `FileList`, so a dropped or pasted file really is in the enclosing `<form>`'s submission |
| Auto upload                                                                                  | K/SF/PrimeNG | Done    | The same input — their `autoUpload` / `auto` is `uploadMode: 'instantly'`                                                                                                                                                          |
| Batch upload                                                                                 | Kendo        | Done    | `batch`                                                                                                                                                                                                                            |
| Sequential vs concurrent upload                                                              | Kendo, SF    | Done    | One `concurrency` number replaces Kendo's `concurrent: false` and SF's `sequentialUpload: true`                                                                                                                                    |
| Chunked upload and its metadata                                                              | K/dx/SF      | Done    | Kendo's `ChunkSettings` defaults and `ChunkMetadata` payload verbatim, so a Kendo-shaped server needs no changes                                                                                                                   |
| Pause / resume                                                                               | Kendo, SF    | Done    | Offered exactly when chunking is on and `resumable`; **derived, not an input** — a whole-file request has nothing to suspend                                                                                                       |
| Retry, automatic and manual                                                                  | Kendo, SF    | Done    | `autoRetry` for whole files, `maxAutoRetries` per chunk, `retry()` for both; a run serving its backoff frees its concurrency slot                                                                                                  |
| Abort / cancel                                                                               | dx, Kendo    | Done    | `abortable`, `abort()`; an abort is not a failure, so it settles without a `failed` the app would have to filter back out                                                                                                          |
| Custom upload handler                                                                        | dx/Prime/Ant | Done    | `uploadAdapter` (`OgeUploadAdapter`), plus `createHttpClientUploadAdapter` for apps whose auth lives in an interceptor                                                                                                             |
| Remove from the server                                                                       | Kendo, SF    | Done    | `removeUrl` / `removeMethod` / `removeHeaders` / `removeField`; only files that reached the server are deleted there                                                                                                               |
| Preloaded / already-uploaded files                                                           | SF, Ant      | Done    | `initialFiles` — SF's `files`, Ant's `defaultFileList`                                                                                                                                                                             |
| File list on/off, per-action toggles                                                         | all, Ant     | Done    | `showFileList` takes the boolean **or** Ant's options object — the house shorthand-or-options idiom                                                                                                                                |
| `listType` text / picture / picture-card                                                     | Ant          | Done    | With PrimeNG's `previewWidth`                                                                                                                                                                                                      |
| Basic versus advanced rendering                                                              | PrimeNG      | Done    | `displayMode: 'full' \| 'compact' \| 'button'`; `compact` is an **OGE extra**                                                                                                                                                      |
| Actions layout                                                                               | Kendo        | Done    | `actionsLayout`                                                                                                                                                                                                                    |
| Templates / slots                                                                            | Prime/SF/Ant | Done    | Six directives; PrimeNG's four icon slots and Ant's three collapse into one `*ogeUploadIconTemplate` with a 14-value discriminated context                                                                                         |
| Localizable strings                                                                          | all          | Done    | Every label, status and validation message is an `OgeUploadMessages` key — absorbing dx's ten string options, PrimeNG's nine and SF's `buttons` object                                                                             |
| Per-file progress                                                                            | all          | Done    | `<oge-progress-bar>` reused rather than rebuilt; its `chunkCount` renders the chunk cursor                                                                                                                                         |
| Transfer rate and time remaining                                                             | —            | Done    | **OGE extra** — on a chunked multi-gigabyte upload a bare percentage says almost nothing                                                                                                                                           |
| Keyboard file removal (`Delete`) and abort (`Escape`)                                        | —            | Done    | **OGE extra** — no reference gives the row actions a keyboard twin                                                                                                                                                                 |
| Roving tab stop over the file list                                                           | —            | Done    | **OGE extra**; `list` / `listitem`, not `grid` — row actions are plain buttons, so axe's `nested-interactive` never fires                                                                                                          |
| Live-region announcements                                                                    | —            | Done    | **OGE extra** — add, reject, start, complete, fail, pause, resume, abort, remove, clear                                                                                                                                            |
| Angular forms: CVA, Signal Forms and `[(value)]` at once                                     | —            | Done    | **OGE extra**; the restrictions attach to the bound control as a plain `ValidatorFn`                                                                                                                                               |
| `@oge-ui/forms` editor type                                                                  | —            | Done    | **OGE extra** — `dataType: 'file'` / `editorType: 'fileUploader'`                                                                                                                                                                  |
| dx `isDirty` / `isValid` / `validationStatus` / `validationErrors`                           | dx           | covered | Re-expressed as read-only signals (`fieldDirty`, `valid`, `validationErrors`) on top of real forms integration                                                                                                                     |
| dx `elementAttr` / `height` / `width` / `visible` / `hint`; SF `htmlAttributes` / `cssClass` | dx, SF       | Skipped | Angular's own class, style and attribute bindings and `@if` cover all of them; `hint` is the native `title`. jQuery-era plumbing                                                                                                   |
| dx `rtlEnabled`                                                                              | dx           | Skipped | House rule: RTL is `[dir='rtl']` in SCSS, never a runtime flag                                                                                                                                                                     |
| dx `focusStateEnabled` / `hoverStateEnabled` / `activeStateEnabled`                          | dx           | Skipped | Focus styling is `:focus-visible` plus the house ring, and is not configurable by design                                                                                                                                           |
| SF `enableHtmlSanitizer`                                                                     | SF           | Skipped | File names render through text interpolation, never `innerHTML` — there is nothing to toggle                                                                                                                                       |
| SF `fileListRendering`                                                                       | SF           | Skipped | Superseded by `*ogeUploadFileTemplate`; a render-interception event is the jQuery answer to a template slot                                                                                                                        |
| dx and SF lifecycle events (`onInitialized`, `onOptionChanged`, `onContentReady`)            | dx, SF       | Skipped | House rule: Angular lifecycle, `effect()` and signals cover them                                                                                                                                                                   |
| Ant `progress` configuration object                                                          | Ant          | Skipped | The visual is `<oge-progress-bar>`, whose `severity`, `showLabel`, `formatLabel` and `chunkCount` already exceed Ant's option bag                                                                                                  |
| Kendo `FileSelect` as a second component                                                     | Kendo        | Skipped | Four of five references express it as a mode; a second component would duplicate the list, restrictions, a11y and docs only to delete the network half                                                                             |
| Client-side hashing or checksums, Web Workers                                                | —            | Skipped | No Worker precedent anywhere in the repo and no APF story for `new Worker(new URL(…))`; chunking is zero-copy `Blob.slice`, so the main thread stays idle                                                                          |
| Automated e2e for a dropped directory                                                        | —            | Skipped | `webkitGetAsEntry()` is absent in jsdom and returns `null` for a synthetic Chromium `DataTransfer`; the files-only fallback is covered, folders are manual                                                                         |
| Drag reordering of the file list                                                             | —            | future  | `sortFiles()` covers programmatic ordering today                                                                                                                                                                                   |

### Second parity sweep (dx Methods/Events pages, Ant's full prop table)

The first sweep missed the reference pages that would not load; these rows are
what the re-read turned up. Three of them were **declared but unimplemented**
in the first pass — options, messages and events existed with no UI behind
them, which is exactly the `partial` state this document forbids.

| Feature                                                                                                                                                             | Reference | Status  | Notes                                                                                                                                                                                                                             |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Preview a file (lightbox)                                                                                                                                           | Ant       | Done    | Row button plus `<oge-modal>`; `previewShowing` is cancelable so an app can open its own viewer. Was declared (`showPreview`, `buttons.preview`, both events) with no UI                                                          |
| Download a file                                                                                                                                                     | Ant       | Done    | Row button; `fileDownloading` is cancelable, default is an anchor click against the server `url` or a temporary object URL. Also declared-with-no-UI in the first pass                                                            |
| `transformFile`                                                                                                                                                     | Ant       | Done    | Ant folds it into `beforeUpload` returning `Promise<File>`; kept separate from `validateFile` so a transform cannot reject and a rejection cannot rewrite. Runs before validation, so the rules judge the bytes that will be sent |
| `previewFile` + `isImageUrl`                                                                                                                                        | Ant       | Done    | One `thumbnailFor` hook — returning `null` is the `isImageUrl: false` half. Resolves after the rows render: a server-rendered PDF thumbnail is worth waiting for, an empty list is not                                            |
| `openFileDialogOnClick`                                                                                                                                             | Ant       | Done    | `false` makes the zone drop-only **and** stops it being a button — a button that does nothing on Enter is worse than none — with the browse button shown instead                                                                  |
| `capture`                                                                                                                                                           | Ant       | Done    | The native attribute, so mobile opens the camera rather than the file browser                                                                                                                                                     |
| `reset(value)`                                                                                                                                                      | dx        | Done    | Back to pristine, clearing touched and dirty. Fires no `clearing` pipeline: a reset is the app rewinding its form, not the user removing files                                                                                    |
| `getFilesData(index)`                                                                                                                                               | SF        | Done    | `getFiles(index?)`                                                                                                                                                                                                                |
| dx index-based `upload(i)` / `abortUpload(i)` / `removeFile(i)`                                                                                                     | dx        | Skipped | Every method takes a `uid` instead. An index is a moving target while a concurrent queue reorders nothing but the app filters the list; the uid is stable                                                                         |
| dx `beginUpdate` / `endUpdate` / `repaint` / `element` / `instance` / `option` / `on` / `off` / `dispose` / `defaultOptions` / `getInstance` / `registerKeyHandler` | dx        | Skipped | The jQuery component lifecycle. Angular's own change detection, bindings, template refs and DI cover every one                                                                                                                    |
| PrimeNG `chooseButtonProps` / `uploadButtonProps` / `cancelButtonProps`                                                                                             | PrimeNG   | Skipped | Pass-through prop bags for its own Button. Here the labels are messages and the whole row or toolbar is replaceable through a template, which is strictly more                                                                    |
| Ant `progress` config, `classNames`, `styles`                                                                                                                       | Ant       | Skipped | Styling escape hatches for a library with no global class names. This suite's `.oge-*` classes are the escape hatch, and the progress visual is `<oge-progress-bar>`                                                              |
| Ant controlled `fileList`                                                                                                                                           | Ant       | covered | `[(value)]` plus the read-only `files` signal; a fully controlled list would mean re-deriving upload state from outside on every keystroke                                                                                        |
