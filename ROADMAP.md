...
| Resources (multi-assignment: labels + dialog tag editor)                 | dx/K      | covered | scalar or array `resourceId` stores; shape-preserving write-back; workload views → backlog                                        |
...
| Work-time calendar (`workCalendar`: working weekdays + holidays)         | K         | covered | shades off days; auto-scheduling rolls starts onto working days, durations kept in working days; per-resource calendars → backlog |
| Export (Excel + drawn PDF)                                               | dx/K      | covered | lazy `@oge-ui/gantt/export-excel` / `export-pdf` entries (optional exceljs/jspdf peers); image export → backlog                   |
