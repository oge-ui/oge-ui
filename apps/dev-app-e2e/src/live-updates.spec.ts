...
test('pushed updates patch grid cells in place and flash them', async ({ page }) => {
  ...
  await expect
    .poll(async () => (await prices.allTextContents()).join('|'), { timeout: 10_000 })
  ...
  const symbols = await grid.locator('.oge-row .oge-cell:first-child').allTextContents();
