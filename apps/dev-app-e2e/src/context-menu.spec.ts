...
test('row and header context menus open with custom items', async ({
  page,
}) => {
  ...
  await expect(
    menu.locator('.oge-menu-item', { hasText: 'Delete (no permission)' }),
  ).toBeDisabled();
  ...
  await headerGrid
    .locator('.oge-header-cell', { hasText: 'Salary' })
    .click({ button: 'right' });
  await expect(menu).toBeVisible();
  await expect(
    menu.locator('.oge-menu-item', { hasText: 'Say hello to Salary' }),
  ).toBeVisible();
  await expect(
    menu.locator('.oge-menu-item', { hasText: 'Pin left' }),
  ).toHaveCount(0);
