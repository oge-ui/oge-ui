...
test('virtualizes 100k rows and shows the right slice at arbitrary positions', async ({
  page,
}) => {
  ...
  await grid
    ...
    .evaluate((el) => (el.scrollTop = 49_999 * 36));
  await expect
    .poll(async () =>
      (
        await grid.locator('.oge-row > .oge-cell:first-child').allTextContents()
      ).map((t) => t.trim()),
    )
    .toContain('50000');
  ...
  await expect(
    grid.locator('.oge-row').last().locator('.oge-cell').first(),
  ).toHaveText('100000');
});

test('measures variable row heights and stays consistent while scrolling', async ({
  page,
}) => {
  await page.goto('/components/data-grid/virtual-scroll');
...
test('virtualizes 200 columns under horizontal scrolling', async ({ page }) => {
  ...
  const firstCell = grid
    .locator('.oge-row')
    .first()
    .locator('.oge-cell:not(.oge-col-spacer)')
    .first();
  const colIndex = Number(
    (await firstCell.getAttribute('data-cell'))?.split('-')[1],
  );
