...
test('operator menu, filter builder and search highlighting work together', async ({ page }) => {
  ...
  await page.locator('.oge-operator-menu .oge-menu-item', { hasText: 'Starts with' }).click();
  ...
  await popup.locator('input.oge-fb-input').fill('100000');
  await popup.getByRole('button', { name: 'Apply' }).click();
  await expect(popup).toHaveCount(0);

  // panel shows the readable filter and the two-way value is displayed
  await expect(page.locator('.oge-filter-panel-text')).toContainText('Greater than or equal');
  ...
  await expect(page.locator('.oge-filter-panel-text')).toContainText('Create filter');
});
...
test('header filter search narrows the distinct values', async ({ page }) => {
  ...
  await expect(popup.locator('.oge-hf-item:not(.oge-hf-all)')).toContainText('Engineering');
});
