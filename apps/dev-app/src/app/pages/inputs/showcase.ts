...
import {
  REACT_INPUTS_SHOWCASE_SECTIONS,
  ReactInputsShowcaseDemos,
} from '../react-inputs/showcase';
...
@Component({
  ...
  template: `
    ...
    >
      @if (fw.isReact()) {
        <p>
          The differentiators, in React idiom: a character counter that counts
          what users perceive (grapheme clusters — emoji families are 1
          character), a password reveal that keeps the caret, one-click copy
          with a live-region announcement, locale-aware number formatting and
          debounced commits (<code>debounce</code>) with a separate
          per-keystroke <code>onInputChange</code> stream. All of it runs the
          same <code>&#64;oge-ui/behavior</code> commit pipeline as the Angular
          editors.
        </p>
      } @else {
        <p>
          The differentiators: a character counter that counts what users
          perceive (grapheme clusters — emoji families are 1 character), a
          password reveal that keeps the caret, one-click copy with a
          live-region announcement, locale-aware number formatting and debounced
          commits with a separate per-keystroke stream.
        </p>
      }
    </app-doc-header>
    ...
    @if (fw.isReact()) {
      <app-react-inputs-showcase-demos />
    } @else {
      <app-demo-card
...
export class InputsShowcasePage {
  ...
  protected readonly reactSections = REACT_INPUTS_SHOWCASE_SECTIONS;
