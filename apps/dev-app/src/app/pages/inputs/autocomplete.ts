...
import {
  REACT_INPUTS_AUTOCOMPLETE_SECTIONS,
  ReactInputsAutocompleteDemos,
} from '../react-inputs/autocomplete';
...
@Component({
  ...
  template: `
    ...
    >
      @if (fw.isReact()) {
        <p>
          <code>&lt;OgeAutocomplete /&gt;</code> from
          <code>&#64;oge-ui/react-inputs</code> is a text editor with a filtered
          ...
          <code>&lt;OgeSelectBox /&gt;</code> when the value must be an item.
        </p>
      } @else {
        <p>
    ...
    @if (fw.isReact()) {
      <app-react-inputs-autocomplete-demos />
    } @else {
      <app-demo-card
        heading="Basic usage"
...
export class InputsAutocompletePage {
  ...
  protected readonly reactSections = REACT_INPUTS_AUTOCOMPLETE_SECTIONS;
