...
import { ReactGridSelectionDemos } from '../react-grid/selection';
...
@Component({
  ...
  template: `
    ...
    @if (fw.isReact()) {
      <app-react-grid-selection-demos />
    } @else {
      <app-demo-card
      ...
      <p>
        With <code>selectionDeferred</code> the grid never materializes a key
        list — the selection <em>is</em> a serializable
        <code>FilterExpr</code> in the two-way
        <code>selectionFilter</code> binding. Select-all over a million remote
        rows costs nothing: the expression captures the current filter, and
      ...
      <ul>
        ...
        <li>
          <kbd>Ctrl</kbd>+<kbd>C</kbd> (or <code>copyToClipboard()</code>)
          copies the selected rows as tab-separated text ready for Excel.
        </li>
        <li>
