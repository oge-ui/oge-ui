...
import { ReactGridPersistenceDemos } from '../react-grid/persistence';
...
@Component({
  ...
  template: `
    ...
    @if (fw.isReact()) {
      <app-react-grid-persistence-demos />
    } @else {
      <app-demo-card
      ...
      <p>
        ...
        <code>stateChange</code> (debounced) to persist on your own terms — e.g.
      ...
      >
        ...
        >
          ...
          <oge-card
            ...
            style="overflow: auto"
            role="complementary"
            aria-label="Last stateChange payload"
          >
            <h3 class="!mt-0 mb-2 text-sm font-semibold">
      ...
      <ul>
        ...
        <li>
          Restore is async-aware: with an API backend the grid renders
          immediately and applies the state when it arrives — unless the user
          already navigated to another <code>stateKey</code>.
        </li>
        <li>
          <code>applyState()</code> also works across grids: capture on one,
          apply to another with the same columns.
        </li>
      </ul>
    }
  `,
