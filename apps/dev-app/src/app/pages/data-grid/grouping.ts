...
import { ReactGridGroupingDemos } from '../react-grid/grouping';
...
@Component({
  ...
  imports: [OgeGrid, OgeColumn, DemoCard, DocHeader, ReactGridGroupingDemos],
  ...
  template: `
    <app-doc-header
      ...
      [chips]="
        fw.isReact()
          ? ['groupPanel', 'groupSummary', 'totalSummary', 'pinned', 'grouping']
          : [
              'groupPanel',
              'groupSummary',
              'totalSummary',
    ...
    @if (fw.isReact()) {
      ...
      <p>
        ...
        expanded — filtered server-side by the group value — and cached for
        later toggles. Expand a department below and watch the skeleton row
      ...
      <ul>
        <li>
          Group aggregates (<code>sum · avg · min · max · count</code>) come
          from the column's <code>groupSummary</code> input;
        ...
        <li>
          ...
          and expects the standard nested <code>GroupedItem</code> payload —
          local and server grouping render identically.
        </li>
        <li>
