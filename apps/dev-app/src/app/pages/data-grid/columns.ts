  ...
  template: `
    ...
    @if (fw.isReact()) {
      ...
      <p>
        ...
        here the department lookup sorts by its Turkish <em>label</em> instead
        of the stored code. <code>calculateFilterExpression</code> rewrites what
        the filter row produces: typing a number into Salary Band filters the
      ...
      <ul>
        <li>
          ...
          display, the filter select, the header filter and the default editor
          all speak the label. CSV export writes the label too.
        </li>
        <li>
          <strong>Calculated</strong> columns (<code>calculateCellValue</code>)
          are display-only: without a <code>field</code> they can't sort or
          filter (server contracts stay serializable).
        </li>
        <li>
