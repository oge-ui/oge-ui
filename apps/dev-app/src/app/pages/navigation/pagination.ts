...
@Component({
  ...
  imports: [DemoCard, DocHeader, PageToc, OgePagination],
  ...
  template: `
    ...
    >
      <p>
        ...
        items" (the grid pager's contracts, kept aligned by design). The numeric
        window holds a <strong>constant width</strong>: ellipsis slots count
        toward <code>maxButtons</code>, so the bar never jitters while paging,
        and an ellipsis never hides a single page.
      </p>
      <p>
        ...
        <code>aria-live="polite"</code> region. Keyboard is the native Tab order
        — every control is a native element, and the APG defines no arrow-key
        behavior to invent. Rendering several bars on one page? Give each a
        distinct <code>paginationLabel</code> via <code>[messages]</code> —
