  ...
  template: `
    ...
    >
      <p>
        ...
        (Neither DevExtreme nor Angular Material/CDK ships a breadcrumb at
        all; the parity references are Kendo and PrimeNG.)
      </p>
      <p>
        When the container runs out of room the <strong>oldest middle</strong>
        crumbs collapse first — the first and last crumb always stay visible —
        and unlike the references the collapsed crumbs stay reachable: the
        ellipsis opens them as real links. The fitting arithmetic is core's
        pure <code>fitToolbarItems</code>, measured against the breadcrumb's
        own box, never the window.
      </p>
    </app-doc-header>
    ...
    >
      <oge-breadcrumb [messages]="{ breadcrumb: 'Breadcrumb — declarative' }">
        <oge-breadcrumb-item text="Home" key="home" url="/components/breadcrumb" />
        <oge-breadcrumb-item
          text="Reports"
          key="reports"
    ...
    <app-demo-card
      [chips]="['ogeBreadcrumbItemTemplate', 'ogeBreadcrumbSeparatorTemplate']"
      heading="Templates"
