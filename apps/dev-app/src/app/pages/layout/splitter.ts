...
import {
  REACT_LAYOUT_SPLITTER_SECTIONS,
  ReactLayoutSplitterDemos,
} from '../react-layout/splitter';
import {
  BASIC_SNIPPET,
  COLLAPSE_SNIPPET,
  CONFIG_SNIPPET,
...
@Component({
  ...
  template: `
    ...
    >
      @if (fw.isReact()) {
        <p>
          <code>&lt;OgeSplitter&gt;</code> from
          <code>&#64;oge-ui/react-layout</code> divides an area into resizable
          panes described by its <code>panes</code> prop — each entry carrying
          its own <code>content</code>, or rendered from one
          <code>renderPane</code> render prop. Sizes are
          <strong>ratios</strong>, not percentages, so a configuration that does
          not add up to 100 is never an error — and a <code>'240px'</code> size
          pins a pane instead.
        </p>
        <p>
          Layout is a single CSS grid in which the separators are real tracks,
          so panes mirror automatically in RTL and no pane needs an inline
          width. The splitter fills its container, so give it a height.
        </p>
      } @else {
        <p>
          ...
          not percentages, so a configuration that does not add up to 100 is
          never an error — and a <code>'240px'</code> size pins a pane instead.
        </p>
        <p>
          Layout is a single CSS grid in which the separators are real tracks,
          so panes mirror automatically in RTL and no pane needs an inline
    ...
    @if (fw.isReact()) {
      <app-react-layout-splitter-demos />
    } @else {
      <app-demo-card
      ...
      >
        <oge-splitter
          class="demo-splitter"
          (paneCollapsed)="onCollapsed($event)"
        >
          <oge-splitter-pane
            key="side"
            [size]="30"
...
export class LayoutSplitterPage {
  ...
  protected readonly sectionsReact = REACT_LAYOUT_SPLITTER_SECTIONS;
