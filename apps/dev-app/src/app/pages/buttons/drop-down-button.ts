...
import {
  REACT_DROP_DOWN_SECTIONS,
  ReactDropDownDemos,
} from '../react-buttons/drop-down';
import {
  ASYNC_SNIPPET,
  BASIC_SNIPPET,
  CONTENT_SNIPPET,
...
@Component({
  ...
  template: `
    ...
    >
      @if (fw.isReact()) {
        <p>
          <code>&lt;OgeDropDownButton /&gt;</code> pairs an
          <code>OgeButton</code> trigger with an anchored menu panel from
          <code>&#64;oge-ui/react-overlay</code> — flip-aware positioning,
          outside-click and Escape closing, focus restore and the full WAI-ARIA
          menu-button keyboard pattern (arrows, Home/End, type-ahead), over the
          same machines and the same stylesheet as the Angular component. Items
          can load lazily from a promise; split mode adds an independent action
          button.
        </p>
      } @else {
        <p>
          ...
          <code>&#64;oge-ui/overlay</code> — flip-aware positioning,
          outside-click and Escape closing, focus restore and the full WAI-ARIA
          menu-button keyboard pattern (arrows, Home/End, type-ahead). Items can
          load lazily from a promise; split mode adds an independent action
          button.
        </p>
      }
    </app-doc-header>
    ...
    @if (fw.isReact()) {
      ...
      <ul>
        ...
        <li>
          <code>holdToConfirm</code>/<code>autoRepeat</code> are not available
          on drop-down buttons.
        </li>
      </ul>
    }
  `,
...
export class ButtonsDropDownPage {
  ...
  protected readonly reactSections = REACT_DROP_DOWN_SECTIONS;
