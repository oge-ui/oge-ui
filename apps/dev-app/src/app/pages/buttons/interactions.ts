  ...
  template: `
    ...
    <ul>
      <li>
        ...
        <code>action</code> manage it. A loading button is disabled and exposes
        <code>aria-busy</code> plus a screen-reader label from the messages
        config (<code>provideOgeButtonsConfig</code>).
      </li>
      <li>
      ...
      <li>
        In gesture modes native clicks are swallowed (and stopped from bubbling)
        so a quick tap can never trigger outer
