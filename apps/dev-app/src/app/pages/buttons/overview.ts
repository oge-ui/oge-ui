...
@Component({
  ...
  imports: [OgeButton, OgeButtonIcon, DemoCard, DocHeader, PageToc],
  ...
  template: `
    ...
    <ul>
      <li>
        Bind <code>(clicked)</code>, not native <code>(click)</code> — only the
        component output goes through <code>clickGuard</code>, the gestures and
        the single-flight <code>action</code> protection.
      </li>
      <li>
        <code>useSubmitBehavior</code> renders <code>type="submit"</code> so the
        button submits its enclosing form; the default is always
        <code>type="button"</code>.
      </li>
      <li>
        Numeric badges join the accessible name via a visually hidden span; the
        dot variant is decorative (<code>aria-hidden</code>).
      </li>
      <li>
        RTL needs no configuration — layout uses logical properties and follows
        the surrounding <code>dir</code> attribute.
      </li>
    </ul>
  `,
})
