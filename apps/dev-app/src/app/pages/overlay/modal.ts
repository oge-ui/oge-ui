...
import {
  REACT_OVERLAY_MODAL_SECTIONS,
  ReactOverlayModalDemos,
} from '../react-overlay/modal';
import {
  BASIC_SNIPPET,
  BUSY_SNIPPET,
  FORM_SNIPPET,
...
@Component({
  ...
  template: `
    ...
    >
      @if (fw.isReact()) {
        <p>
          <code>&lt;OgeModal&gt;</code> is the centered dialog primitive:
          backdrop, focus trap, body scroll lock, Escape/backdrop closing and
          focus restore. Content renders lazily behind the controlled
          <code>opened</code> prop and joins the shared overlay Escape stack, so
          popups opened inside the modal close before the modal itself. Declare
          it near the component root — <code>transform</code>ed ancestors break
          <code>position: fixed</code> — or open one imperatively through
          <code>useOgeModals()</code>.
        </p>
      } @else {
        <p>
          <code>oge-modal</code> is the centered dialog primitive: backdrop,
          focus trap, body scroll lock, Escape/backdrop closing and focus
          restore. Content renders lazily behind the <code>opened</code> model
          and joins the shared overlay Escape stack, so popups opened inside the
          modal close before the modal itself. Declare it near the component
          root — <code>transform</code>ed ancestors break
    ...
    @if (fw.isReact()) {
      ...
      >
        ...
        >
          ...
          <p class="m-0">
            Drag the title bar; resize from the bottom-right corner. Position
            and size reset on reopen (<code>restorePosition</code>). The
    ...
    <ul>
      ...
      @if (fw.isReact()) {
        <li>
          Headerless modals (<code>showCloseButton={{ '{' }}false{{ '}' }}</code
          >, no title) should set <code>ariaLabel</code>;
          <code>padding={{ '{' }}false{{ '}' }}</code> makes the body flush for
          grids and custom layouts.
        </li>
        <li>
          The ✕ button's aria label localizes via
          <code
            >&lt;OgeOverlayConfigProvider config={{ '{{' }} messages: &#123;
            modalClose: '…' &#125; {{ "}}" }}&gt;</code
          >
          or the per-instance <code>messages</code> prop.
        </li>
        <li>
          Body scroll locks while open (scrollbar-width compensated, ref-counted
          across stacked modals) — disable with
          <code>scrollLock={{ '{' }}false{{ '}' }}</code
          >.
        </li>
      } @else {
        <li>
          Headerless modals (<code>showCloseButton=false</code>, no title)
          should set <code>ariaLabel</code>; <code>[padding]="false"</code>
          makes the body flush for grids and custom layouts.
        </li>
        <li>
          The ✕ button's aria label localizes via
          <code
...
export class OverlayModalPage {
  ...
  protected readonly reactSections = REACT_OVERLAY_MODAL_SECTIONS;
