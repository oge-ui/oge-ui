...
import {
  REACT_OVERLAY_TOAST_SECTIONS,
  ReactOverlayToastDemos,
...
@Component({
  ...
  imports: [OgeButton, DemoCard, DocHeader, PageToc, ReactOverlayToastDemos],
  ...
  template: `
    <app-doc-header
      ...
      [chips]="
        fw.isReact()
          ? ['useOgeToasts', 'promise()', 'coalesce', 'action']
          : ['OgeToastService', 'promise()', 'coalesce', 'action']
      "
    >
      @if (fw.isReact()) {
        <p>
          <code>useOgeToasts()</code> — under an
    ...
    <ul>
      @if (fw.isReact()) {
        <li>
          Defaults live in
          <code
            >&lt;OgeOverlayConfigProvider config={{ '{{' }} toastPosition,
            toastDisplayTime, toastMaxVisible, toastProgressBar,
            toastCoalesceDuplicates {{ "}}" }}&gt;</code
      ...
      @if (fw.isReact()) {
        <li>
          <code>renderContent</code> replaces the body for fully custom content
          — heavy interactive flows belong in
          <code>useOgeModals()</code> instead.
        </li>
      } @else {
        <li>
          <code>template</code> replaces the body for fully custom content —
          heavy interactive flows belong in <code>OgeModalService</code>
          instead.
        </li>
      }
    </ul>
...
export class OverlayToastPage {
  ...
  protected readonly reactSections = REACT_OVERLAY_TOAST_SECTIONS;
