...
import {
  REACT_UPLOAD_OVERVIEW_SECTIONS,
  ReactUploadOverviewDemos,
} from '../react-upload/overview';
import {
  CHUNK_SNIPPET,
...
@Component({
  ...
  template: `
    ...
    >
      @if (fw.isReact()) {
        <p>
          ...
          the controlled <code>value</code> pair as plain
          <code>File</code> objects. Add a URL and the same element uploads —
          through a pluggable adapter, chunked and resumable on request. It runs
          the same <code>&#64;oge-ui/behavior</code> engine and list machine as
          the Angular component and loads the same stylesheet.
        </p>
        <p>
          <strong>XHR, not <code>fetch</code></strong
          >: <code>xhr.upload.onprogress</code> is the only browser API that
          reports request-body progress, which is why every reference library
          uses it too. Swap the transport for a subtree with
          <code>&lt;OgeUploadTransportProvider&gt;</code>.
        </p>
        <p>
          ...
          <code>&lt;input type="file"&gt;</code> stays in the accessibility
          tree, the drop zone <em>is</em> a button, the list is a
          <code>list</code> with a roving tab stop, <kbd>Delete</kbd> removes
          the focused row, and every change is announced politely.
        ...
        <p>
          ...
          pasted screenshots included), restrictions that stay on the row with
          their reason, image previews, and everything chosen exposed through
          <code>[(value)]</code> as plain <code>File</code> objects. Add a URL
          and the same element uploads — through a pluggable adapter, chunked
          and resumable on request.
        </p>
        <p>
          <strong>XHR, not <code>fetch</code></strong
          >: <code>xhr.upload.onprogress</code> is the only browser API that
          reports request-body progress, which is why every reference library
          uses it too. Apps whose auth lives in an interceptor can swap in
        ...
        <p>
          ...
          contract is assembled from primitives and written down: the real
          <code>&lt;input type="file"&gt;</code> stays in the accessibility
          tree, the drop zone <em>is</em> a button (so the keyboard reaches the
          same dialog and nothing nests inside an interactive element), the list
          is a <code>list</code> with a roving tab stop,
          <kbd>Delete</kbd> removes the focused row, and every change is
          announced politely.
          <a routerLink="/components/upload/api">Full API reference →</a>
        </p>
      }
    ...
    @if (fw.isReact()) {
      <app-react-upload-overview-demos />
    } @else {
      <app-demo-card
...
export class UploadOverviewPage {
  ...
  protected readonly reactSections = REACT_UPLOAD_OVERVIEW_SECTIONS;
