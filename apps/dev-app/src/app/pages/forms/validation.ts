...
import {
  REACT_FORMS_VALIDATION_SECTIONS,
  ReactFormsValidationDemos,
...
@Component({
  ...
  template: `
    <app-doc-header
      ...
      [chips]="
        fw.isReact()
          ? ['validationRules', 'shared evaluator', 'summary', 'focus']
          : ['validationRules', 'Signal Forms', 'summary', 'focus']
      "
    >
      @if (fw.isReact()) {
        <p>
          There is exactly one rule evaluator in the suite, and it is
          framework-free: <code>evaluateOgeValidationRules()</code> in
          <code>&#64;oge-ui/behavior</code>. The React form calls it directly;
          the Angular form compiles the same rules into a Signal Forms schema
          whose <code>validate()</code> calls that very function. A rule
          therefore cannot mean two different things in the two layers.
        </p>
        <p>
          What the React layer does not have is Angular's two alternative
          bindings — <code>[fieldTree]</code> and <code>[formGroup]</code> — and
          the schema-carried layout that rides on the first of them. Those three
          sections of this page are Angular-only by construction
          (<code>docs/REACT-PARITY.md</code>); everything else below is the same
          page.
        </p>
      } @else {
        <p>
          ...
          <code>minLength()</code>/<code>maxLength()</code>, and so on. Nothing
          in <code>&#64;oge-ui/forms</code> re-implements validation.
        </p>
        <p>
          Which means the rules only apply where the form owns the model, i.e.
          <code>[(formData)]</code>. Bind <code>[fieldTree]</code> or
          <code>[formGroup]</code> and the caller owns validation; the form
          warns in dev mode if you set rules anyway.
        </p>
      }
    </app-doc-header>
    ...
    @if (fw.isReact()) {
      <app-react-forms-validation-demos />
    } @else {
      <app-demo-card
...
export class FormsValidationPage {
  ...
  protected readonly reactSections = REACT_FORMS_VALIDATION_SECTIONS;
