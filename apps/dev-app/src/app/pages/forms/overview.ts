...
import {
  REACT_FORMS_OVERVIEW_SECTIONS,
  ReactFormsOverviewDemos,
} from '../react-forms/overview';
import {
  ACTIONS_SNIPPET,
  BASIC_SNIPPET,
  EDITOR_SNIPPET,
...
@Component({
  ...
  template: `
    <app-doc-header
      ...
      [chips]="
        fw.isReact()
          ? ['controlled model', 'render props', 'container queries', 'a11y']
          : ['Signal Forms', 'reactive forms', 'container queries', 'a11y']
      "
    >
      @if (fw.isReact()) {
        <p>
          <code>&lt;OgeForm /&gt;</code> from
          <code>&#64;oge-ui/react-forms</code> lays out the
          <code>&#64;oge-ui/react-inputs</code> editors: labels, responsive
          columns, nestable <code>&lt;fieldset&gt;</code> groups, a validation
          summary and submit. Items come from the nested
          <code>layout</code> array, from a data-driven <code>items</code>
          array, or both — the layout first.
        </p>
        <p>
          The model is one controlled pair: <code>formData</code> +
          <code>onFormDataChange</code> (or the uncontrolled
          <code>defaultFormData</code>). Validation is not a second engine
          either — each item's <code>validationRules</code> run through
          <code>evaluateOgeValidationRules()</code> in
          <code>&#64;oge-ui/behavior</code>, the very function the Angular
          form's schema calls, so a rule can never be worded differently in the
          two layers.
        </p>
      } @else {
        <p>
          ...
          nestable <code>&lt;fieldset&gt;</code> groups, a validation summary
          and submit. Items come from projected
        ...
        <p>
          ...
          <code>[(formData)]</code> for a plain signal model. That last one is
          not a second validation engine — the form compiles each item's
    ...
    @if (fw.isReact()) {
      <app-react-forms-overview-demos />
    } @else {
      <app-demo-card
...
export class FormsOverviewPage {
  ...
  protected readonly reactSections = REACT_FORMS_OVERVIEW_SECTIONS;
