...
describe('OgeGrid windowed scrolling (remote virtual / infinite)', () => {
  async function render(withTotal = true): Promise<{
    ...
    const source = new CustomDataSource<Row>({
      ...
      load: async (options) => {
        ...
        const data = Array.from({ length: Math.max(0, Math.min(take, TOTAL - skip)) }, (_, i) => ({
          id: skip + i + 1,
          name: `Row ${skip + i + 1}`,
        }));
    ...
    fixture.componentRef.setInput('scrolling', { mode: 'virtual', remote: true });
  ...
  it('shows skeleton fillers, then real rows after a far jump — skipping middle blocks', async () => {
    ...
    (grid as unknown as { scrollTop: { set(v: number): void } }).scrollTop.set(50_000 * 30);
    fixture.detectChanges();
  ...
  it('re-requests visited blocks never, and invalidates the cache on sort', async () => {
    const { fixture, el, grid, log } = await render();
    const scroll = (grid as unknown as { scrollTop: { set(v: number): void } }).scrollTop;
    ...
    el.querySelectorAll('.oge-header-cell')[0].dispatchEvent(new MouseEvent('click', { bubbles: true }));
    await settle(fixture);
