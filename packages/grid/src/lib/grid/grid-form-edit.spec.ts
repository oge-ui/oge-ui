...
describe('OgeGrid form edit mode', () => {
  it('swaps the row for an inline labeled form and commits on save', async () => {
    ...
    // the fields are laid out by <oge-form>, so the labels are the editors' own
    const labels = Array.from(form.querySelectorAll('.oge-input-label')).map(
      (l) => l.textContent?.trim(),
    );
    expect(labels).toEqual(['Name', 'Salary']);
    // the edited row's cells are replaced; the other row still renders cells
    expect(el.querySelectorAll('.oge-row:not(.oge-edit-form-row)').length).toBe(
      1,
    );
