...
export interface OgeExcelExportOptions extends OgeExportOptions {
...
export function buildExcelWorkbook<T>(
  ...
  for (const row of data.rows) {
    sheet.addRow(data.columns.map((column) => cellValue(column, row)));
  }
...
export async function exportGridToExcel<T extends object>(
  ...
  const workbook = buildExcelWorkbook(await grid.getExportData({ scope: options.scope }), options);
