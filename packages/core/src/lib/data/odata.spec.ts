...
describe('buildODataQuery', () => {
  it('serializes paging, sorting and count', () => {
    ...
    expect(query).toBe(
      `$skip=40&$top=20&$orderby=${encodeURIComponent('name,price desc')}&$count=true`,
    );
  });
  ...
  it('folds searchText over searchFields into the filter', () => {
    const query = buildODataQuery(
      {
        searchText: 'ada',
        filter: { type: 'binary', field: 'active', op: 'eq', value: true },
      },
      { searchFields: ['firstName', 'lastName'] },
    );
    const filter = decodeURIComponent(query.replace('$filter=', ''));
    expect(filter).toBe(
      "(active eq true) and ((contains(firstName,'ada') or contains(lastName,'ada')))",
    );
  });
...
describe('ODataDataSource', () => {
  ...
  it('throws a descriptive error on HTTP failure', async () => {
    ...
    await expect(source.load({})).rejects.toThrow(
      'ODataDataSource: 500 Server Error',
    );
  });
});
