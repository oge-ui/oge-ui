/**
 * The item model of the forms family, framework-free (ADR 0001): label
 * derivation, dot-path access, data-type inference, the editor precedence
 * rule, per-item resolution, `visibleIndex` ordering and the layout column
...
/** The empty value `clear()` writes for an item, by data type. */
export function emptyValueForDataType(dataType: OgeFormDataType): unknown {
  switch (dataType) {
    case 'boolean':
      return false;
    case 'number':
...
/**
 * `grid-template-columns` for a column count. `'auto'` (and an unset count)
 * fits as many `--oge-form-min-col` columns as the form's own width allows —
 * a container query, never the window's width.
 */
export function formColumnsCss(colCount: OgeFormColCount | undefined): string {
...
/**
 * How many columns an item's `colSpan` may be clamped against. `'auto'` has no
 * fixed count, so it reports the 12 the CSS grid can never exceed in practice.
 */
export function formColumnsCount(
  colCount: OgeFormColCount | undefined,
  fallback: OgeFormColCount,
): number {
  const effective = colCount ?? fallback;
