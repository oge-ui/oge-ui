/**
 * The uploader's message catalog and package defaults, shared by both render
 * layers (ADR 0001) so the Angular `provideOgeUploadConfig()` and the React
...
/**
 * One message per `OgeUploadErrorKind`.
 *
...
/** Package-wide defaults for every uploader under a provider. */
...
export function mergeUploadMessages(
  base: OgeUploadMessages,
  input: OgeUploadMessagesInput | undefined,
): OgeUploadMessages {
  ...
  return {
    ...
    dropZone: { ...base.dropZone, ...input.dropZone },
...
export function resolveUploadConfig(
  input: OgeUploadConfigInput | undefined,
  base: OgeUploadConfig = OGE_DEFAULT_UPLOAD_CONFIG,
): OgeUploadConfig {
  const { messages, ...rest } = input ?? {};
  return {
    ...base,
    ...rest,
    messages: mergeUploadMessages(base.messages, messages),
  };
}
