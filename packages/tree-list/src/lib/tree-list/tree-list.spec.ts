...
function rowByTitle(el: HTMLElement, title: string): HTMLElement | undefined {
  return Array.from(el.querySelectorAll<HTMLElement>('.oge-row')).find((row) =>
    (row.querySelector('.oge-tree-cell-text')?.textContent ?? '').includes(title)
  );
}
...
describe('OgeTreeList', () => {
  ...
  it('renders only the roots when collapsed by default', async () => {
    ...
    expect(el.querySelector('.oge-viewport')?.getAttribute('role')).toBe('treegrid');
  });
  ...
  it('autoExpandAll renders the whole hierarchy', async () => {
    ...
    expect(rowTitles(el)).toEqual(['Root A', 'Child A1', 'Grand A1a', 'Child A2', 'Root B']);
  });
  ...
  it('sorts siblings within their parent, preserving the hierarchy', async () => {
    ...
    const orderHeader = Array.from(el.querySelectorAll<HTMLElement>('.oge-header-cell')).find(
      (cell) => cell.textContent?.includes('Order')
    );
    orderHeader?.click();
    await settle(fixture);
    // asc by "order": Root B (1) before Root A (2); Child A2 (1) before Child A1 (2)
    expect(rowTitles(el)).toEqual(['Root B', 'Root A', 'Child A2', 'Child A1', 'Grand A1a']);
  ...
  it('discards orphan rows by default', async () => {
    const { el } = await render((host) => {
      host.data = [...TASKS.map((task) => ({ ...task })), { id: 9, parentId: 99, title: 'Orphan', order: 1 }];
