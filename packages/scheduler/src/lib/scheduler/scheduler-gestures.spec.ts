...
describe('<oge-scheduler> gestures', () => {
  ...
  it('a sub-threshold press never commits', async () => {
    ...
    document.dispatchEvent(
      pointer('pointermove', { clientX: 351, clientY: 91 }),
    );
    document.dispatchEvent(pointer('pointerup', {}));
    await settle(fixture);
    expect(fixture.componentInstance.updated.length).toBe(0);
  ...
  it('resize via the end handle grows the appointment', async () => {
    const handle = host.querySelector<HTMLElement>('.oge-scheduler-resize-end');
    handle?.dispatchEvent(
      pointer('pointerdown', { clientX: 350, clientY: 120, button: 0 }),
    );
    document.dispatchEvent(
      pointer('pointermove', { clientX: 350, clientY: 180 }),
    );
    document.dispatchEvent(pointer('pointerup', {}));
    await settle(fixture);
    const updated = fixture.componentInstance.updated.at(-1)?.appointmentData;
  ...
  it('keyboard move (Ctrl+Arrow) and resize (Ctrl+Shift+Down) commit directly', async () => {
    ...
    stubRowsRect(host);
    host.querySelector<HTMLElement>('.oge-scheduler-chip-box')?.dispatchEvent(
      new KeyboardEvent('keydown', {
        key: 'ArrowDown',
        ctrlKey: true,
        shiftKey: true,
  ...
  it('month-view bar drag moves by whole days, preserving the time', async () => {
    ...
    await settle(fixture);
    const gridEl = host.querySelector<HTMLElement>('.oge-scheduler-month-area');
    ...
    bar?.dispatchEvent(
      pointer('pointerdown', { clientX: 350, clientY: 150, button: 0 }),
    );
    document.dispatchEvent(
      pointer('pointermove', { clientX: 50, clientY: 250 }),
    );
    document.dispatchEvent(pointer('pointerup', {}));
    await settle(fixture);
    const updated = fixture.componentInstance.updated.at(-1)?.appointmentData;
